# -*- coding: utf-8 -*-
"""
Created on Mon Dec 19 20:08:35 2022
...
def cross_validation_of_models(models,x_train_res,y_train_res):    
    ...
    #sort by F1 Score
    cv_scores = cv_scores.sort_values(by='F1 Score', ascending=False)
...
def hyperparameter_tuning(models,param_grid,x_train, y_train):
    ...
    for index, model in enumerate(models):
        ...
        # RandomizedSearchCV
        cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=1)
        rs = GridSearchCV(model[1], param_grid[index][1], scoring='f1', cv=cv, n_jobs=-1)
...
def classification_prediction(models, x_train, x_test, y_train, y_test):
    ...
    for index, (name,model) in enumerate(models):
        ...
        plt.show()
        #save image
        fig.savefig(f'./images/5. Modeling/{name}.png', bbox_inches='tight')
        
    #sort by F1 Score
    prediction_results = prediction_results.sort_values(by=['F1 Score','Recall','Accuracy'], ascending=False)
